...
fileprivate let lengthyLength = 75
fileprivate let lengthyData = Data(repeating: 0xff, count: lengthyLength)
fileprivate let lengthyData2 = Data(repeating: 0xff, count: lengthyLength - 1) + Data([0x00])

fileprivate let helloData = "hello".data(using: .ascii)!
fileprivate let expectedRIPEMD160 = Data([0x10, 0x8f, 0x07, 0xb8, 0x38, 0x24, 0x12, 0x61, 0x2c, 0x04, 0x8d, 0x07, 0xd1, 0x3f, 0x81, 0x41, 0x18, 0x44, 0x5a, 0xcd])
fileprivate let expectedSHA1 = Data([0xaa, 0xf4, 0xc6, 0x1d, 0xdc, 0xc5, 0xe8, 0xa2, 0xda, 0xbe, 0xde, 0x0f, 0x3b, 0x48, 0x2c, 0xd9, 0xae, 0xa9, 0x43, 0x4d])
fileprivate let expectedSHA256 = Data([0x2c, 0xf2, 0x4d, 0xba, 0x5f, 0xb0, 0xa3, 0x0e, 0x26, 0xe8, 0x3b, 0x2a, 0xc5, 0xb9, 0xe2, 0x9e, 0x1b, 0x16, 0x1e, 0x5c, 0x1f, 0xa7, 0x42, 0x5e, 0x73, 0x04, 0x33, 0x62, 0x93, 0x8b, 0x98, 0x24])
...
struct ScriptOpsTests {
    ...
    @Test("Lengthy data", arguments: [
        ...
        // size
        ([Data](), [ScriptOp.pushBytes(lengthyData), .size], [lengthyData, try! ScriptNumber(lengthyLength).data]),
        // equal
        ([Data](), [ScriptOp.pushBytes(lengthyData), ScriptOp.pushBytes(lengthyData), .equal], [ScriptNumber.one.data]),
        ([Data](), [ScriptOp.pushBytes(lengthyData), ScriptOp.pushBytes(lengthyData2), .equal, .constant(1)], [ScriptNumber.zero.data, ScriptNumber.one.data]),
    ])
